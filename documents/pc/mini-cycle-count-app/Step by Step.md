...
### Screen 2: Upload Item Buy Detail + Precount
- **Upload Item Buy Detail**: DRS Excel report (last 18 months of purchases)
  - File: `Item Buy Detail Aug124-April142026.xlsx` (Excel format)
  - Important: First column header says "Ski" but actually means "SKU"
  - Key columns: SKI (SKU), Brand Description, Model, Description, Buy Date, QOH, DRS Cost, DRS Retail
- **Upload Precount File**: CSV of what DRS thinks is on hand
  - File: `Precount.001` (CSV format)
- "Continue" button (disabled until both files uploaded)

### Screen 3: Upload Store Scans
- **Upload Scanned Items**: CSV from physical count
  - File: `BShortsPhysicalCountRev3.csv` (or similar)
  - Format: CSV with no header row
  - Column 1: SKU scanned
  - Column 2: Marks beginning/end of count zones
- "Continue" button

### Screen 4: Review Missing Items
- Display scrollable SKU list showing:
  - SKU, Description/Brand, Date bought, DRS Cost
- Per-item actions:
  - **Accept**: Mark item as written off (loss)
  - **Skip**: Keep item in inventory (don't write off)
- Bulk action: **"Trust the Process"** - accept all as writeoffs without reviewing
- "Continue" button (to download step)

### Screen 5: Download Missing SKUs (Mandatory)
- Display summary: count of items being written off
- **Download CSV**: `shorts-writeoff-list.csv` (list of SKUs approved for write-off)
- "Finish" button

### Screen 6: Generate Barcodes (Optional)
- **Upload SKU list** to barcode generator
- **Download formatted PDF**: 5×22 grid (91 barcodes per page)
- User then prints and scans in DRS "Adjust Out to Zero"

---

## Full Process Details (Reference)
